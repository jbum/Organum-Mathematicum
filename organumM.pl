...
while ($_ = shift) 
{
  if (/^-m/) {
    ...
    print <<EOT;
ORGANUM - Jim Bumgardner, data by Athanasius Kircher, Kaspar Schott
 Options: 
    ...
    foreach $ni (0..$#{$pf})
    {
      if ($useBarLines) {
        ...
        if ($pLength + $divLength > $barLength)
        {
          ...
          printf OFILE '%s%s ', TableToPitch($vn,$mi,$pf->[$ni]), TableToNotelength($rf->[$ni+$rOffset]);
        }
        $pLength += $divLength;
...
sub TableToMIDI()
{
  ...
  if ($lastPitchM > 0 && (defined $lastPitchM) &&
      abs(($lastPitchM % 12) - ($n % 12)) <= $vleading && 
...
sub TableToPitch()
{
  ...
  if ($lastPitchN > 0 && (defined $lastPitchN) &&
      abs(($lastPitchN % 12) - ($n % 12)) <= $vleading && 
